...
N_SPLITS_KFold = 5
OPTUNA_N_TRIALS = 30  # (성능 향상을 위해 100회 이상 권장)
...
# -------------------\
# [V9] 학습/로드 (누수 피처 'Test_x', 'Test_y' 제거)
# -------------------\
def fit_or_load(
    ...
    if len(df_idx) and label_col in df_idx.columns:
        ...
        # [V9 수정] 'Test_x', 'Test_y'를 drop_cols에 추가하여 피처로 사용되는 것을 방지
        drop_cols = [key, label_col, "PrimaryKey"] + \
                    (["Test"] if "Test" in df.columns else []) + \
                    ['Test_x', 'Test_y'] # merge 시 생성되는 중복 컬럼 제거
        
        # [V8] 모델과 관련 없는 피처(Column)를 명시적으로 제거
        if which == "A":
...
def predict_partition(
    ...
    # [V9 수정] 'Test_x', 'Test_y'를 drop_cols에 추가
...
def save_meta():
    meta = dict(
        model="HGB(3-seed soft ensemble) + OrdinalEnc + Calibration [Optuna V9 - TimeSeries FE + GroupKFold + max_iter tuning + A/B Split + LeakFix]", # [V9] 모델명
        feature_engineering=[
            ...
            "[V8-FE] A/B Feature Splitting: Model A trains only on A* features, Model B on B* features.",
            "[V9-Fix] Removed 'Test_x', 'Test_y' leakage features from training." # [V9]
        ],
...
def main():
    ...
    # --- 추론 ---
    # [V9] 수정된 predict_partition 함수가 호출됨
    ...
    save_meta() # [V9] 메타 정보 저장

    dt = time.time() - t0
    print(f"[V9] submission saved -> {SUBMISSION_PATH} | elapsed: {dt:.2f}s")

...
def build_preprocessor(num_cols: List[str], cat_cols: List[str]) -> ColumnTransformer:
    # [수정] v2(v5) 방식 : Imputer를 다시 사용하여 교차 NaN 문제를 방지합니다.
    ...
    preproc = ColumnTransformer(
        transformers=[
            ("num", numeric_pipe, num_cols), # [수정] passthrough -> numeric_pipe
...
def save_meta():
    meta = dict(
        model="HGB(3-seed soft ensemble) + OrdinalEnc + Calibration [Optuna Tuned V5 - FE + HPO + Imputer]", # [수정] 모델명
        feature_engineering=[

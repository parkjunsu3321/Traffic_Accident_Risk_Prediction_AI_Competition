...
from sklearn.model_selection import train_test_split, GroupKFold # [V6] GroupKFold 임포트
...
CALIB_CV = 3                    
N_SPLITS_KFold = 5 # [V6] GroupKFold 분할 수
...
def create_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # [V6] 시계열 피처를 만들기 위해 먼저 정렬
    df_proc = df_proc.sort_values(by=['PrimaryKey', 'TestDate_num'])
    
    g = df_proc.groupby('PrimaryKey') # [V6] 그룹 객체 (초기 정의)
    ...
    # [V6-FIX] 'g'를 여기서 다시 정의해야 NA_COUNT 등 신규 컬럼을 인식합니다.
    ...
    for col in key_numeric_cols:
        # diff: 이번 검사 - 이전 검사 (변화량)
        df_proc[f'{col}_diff'] = g[col].diff()
        # shift1: 이전 검사 값
        df_proc[f'{col}_shift1'] = g[col].shift(1)
        
        # rolling3_mean: 최근 3회 검사 평균 (본인 포함)
        roll_mean = g[col].rolling(3, min_periods=1).mean()
        # [V6-FIX] MultiIndex를 reset하여 df_proc의 인덱스와 맞춥니다.
        df_proc[f'{col}_roll3_mean'] = roll_mean.reset_index(level=0, drop=True)

    # [V6] 시계열 피처 생성 시 발생하는 NaN (첫 번째 검사)은
    # 이후 build_preprocessor의 SimpleImputer(median)가 알아서 처리해줌.
...
def fit_or_load(
    ...
    if len(df_idx) and label_col in df_idx.columns:
        ...
        groups = df['PrimaryKey'].values # [V6] GroupKFold를 위한 그룹

        # [V6] train_test_split을 GroupKFold로 대체 (정보 유출 방지)
        ...
        # GKF는 인덱스를 반환. 전체 X, y에서 첫 번째 분할을 사용.
        # 이 분할은 Optuna 튜닝과 최종 앙상블 홀드아웃 점수 계산에 사용됨
        ...
        def objective(trial):
            ...
            # [V6] Optuna도 GroupKFold로 분리된 단일 검증셋(X_val_t)을 사용
...
def save_meta():
    meta = dict(
        model="HGB(3-seed soft ensemble) + OrdinalEnc + Calibration [Optuna Tuned V6 - TimeSeries FE + GroupKFold]", # [V6] 모델명
        feature_engineering=[
            ...
            "[V6-FE] Time-Series features (diff, shift, roll3_mean) for key numeric cols" # [V6]
        ],
...
def main():
    ...
    # [V6] 시계열 피처가 포함된 create_features 함수가 여기서 호출됨
    ...
    # [V6] GroupKFold가 적용된 fit_or_load 함수가 호출됨
    ...
    dt = time.time() - t0
    print(f"[V6] submission saved -> {SUBMISSION_PATH} | elapsed: {dt:.2f}s")

...
from sklearn.calibration import CalibratedClassifierCV, calibration_curve # [V12] calibration_curve 임포트
...
CALIB_CV = 3
N_SPLITS_KFold = 5
OPTUNA_N_TRIALS = 50 # 실제 사용 시 50~100회 권장
...
def create_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # [V11.1] 'g' 객체를 NA_COUNT 등이 추가된 'df_proc'로 새로고침
...
# -------------------\
# [V12] 단일 Fold 학습 함수 (Optuna 목표 수정)
# -------------------\
def train_single_fold(
    ...
    def objective(trial):
        ...
        ece = expected_calibration_error(y_val, val_proba) # 추가된 ECE 함수 사용
        
        # Score = 0.5 * (1 - AUC) + 0.25 * Brier + 0.25 * ECE
        final_score = 0.5 * (1.0 - auc) + 0.25 * brier + 0.25 * ece
        
        return final_score # [V12] AUC 대신 최종 점수 반환

    # [V12] direction을 "minimize"로 변경 (점수가 낮을수록 좋음)
    study = optuna.create_study(direction="minimize") 
    study.optimize(objective, n_trials=OPTUNA_N_TRIALS)

    best_params = study.best_params
    # [V12] 로그 메시지 수정
    print(f"{fold_label} Optuna finished. Best Score (minimize): {study.best_value:.5f}")
    ...
    try:
        ...
        # [V12] 최종 검증 로그에도 모든 지표 표시
        auc_final = roc_auc_score(y_val, val_proba)
        brier_final = brier_score_loss(y_val, val_proba)
        ece_final = expected_calibration_error(y_val, val_proba)
        score_final = 0.5 * (1.0 - auc_final) + 0.25 * brier_final + 0.25 * ece_final
        
        print(f"{fold_label} Holdout Final Score={score_final:.5f} (AUC={auc_final:.5f}, Brier={brier_final:.5f}, ECE={ece_final:.5f})")
    except Exception as e:
...
def save_meta():
    meta = dict(
        model=f"HGB({N_SPLITS_KFold}-Fold Ensemble) + 3-seed AvgProba + OrdinalEnc + Calib [Optuna V12]", # [V12]
        
        # [V12] Optuna 목표 명시
        optuna_objective="0.5*(1-AUC) + 0.25*Brier + 0.25*ECE (Minimize)",
...
def main():
    ...
    if len(A_train_idx) > 0:
        ...
        for fold_id, (train_indices, val_indices) in enumerate(gkf_A.split(X_A_full, y_A, groups_A)):
            ...
            if not (os.path.exists(A_MODEL_PATH) and os.path.exists(A_PREPROC_PATH)):
                ...
                # [V12] 수정된 train_single_fold 함수 호출
                train_single_fold(
    ...
    save_meta() # [V12] 메타 정보 저장

    dt = time.time() - t0
    print(f"[V12] submission saved -> {SUBMISSION_PATH} | elapsed: {dt:.2f}s")

...
CALIB_CV = 3
N_SPLITS_KFold = 5
OPTUNA_N_TRIALS = 30  # (K-Fold로 시간이 5배 걸리므로 30 유지. 시간되면 100+)
optuna.logging.set_verbosity(optuna.logging.WARNING)

ENSEMBLE_SEEDS: Sequence[int] = (42, 202, 777)

# [V10] Optuna가 Fold별로 튜닝할 '기본' 파라미터
...
# [V10] Optuna가 찾은 Fold별 파라미터를 받도록 수정
...
# -------------------\
# [V10] 단일 Fold 학습 함수 (V9의 fit_or_load 학습 로직)
# -------------------\
def train_single_fold(
    ...
    # 3. 전처리기 (Preprocessor)
    # [V9] drop_cols는 비어있음 (이미 X_full에서 'Test_id' 등 제거됨)
    ...
    # [V10] 전역 BASE_HGB_PARAMS를 복사하여 이 Fold에서만 사용할 파라미터 셋 생성
    ...
    def objective(trial):
        ...
        hgb_params = fold_hgb_params.copy() # BASE가 아닌 fold_hgb_params 복사
    ...
    # [V10] Optuna 결과를 이 Fold의 파라미터에만 업데이트 (전역 X)
    ...
    try:
        ...
        print(f"{fold_label} Holdout AUC={auc:.5f}, Brier={brier:.5f}")
    except Exception as e:
...
def save_meta():
    meta = dict(
        model=f"HGB({N_SPLITS_KFold}-Fold Ensemble) + 3-seed AvgProba + OrdinalEnc + Calib [Optuna V10]", # [V10] 모델명
        feature_engineering=[
        ...
        validation_strategy=f"GroupKFold (n_splits={N_SPLITS_KFold}) on PrimaryKey. Full K-Fold Ensemble.", # [V10]
        
        # [V10] Optuna는 Fold별로 수행되므로 '기본' 파라미터만 저장
...
def main():
    ...
    save_meta() # [V10] 메타 정보 저장

    dt = time.time() - t0
    print(f"[V10] submission saved -> {SUBMISSION_PATH} | elapsed: {dt:.2f}s")

...
def create_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # --- ⬇️ [V14] B-피처 상호작용 (A9 프록시) ⬇️ ---
    print("[Global FE V14] Creating B-feature interactions (A9 proxies)...")
    b_proxy_cols = []
    
    # B4(선택) - B3(단순) 반응시간 차이 (인지 부하)
    if 'B4-2' in df_proc.columns and 'B3-2' in df_proc.columns:
        # B3-2, B4-2 are already numeric from rt_cols loop
        df_proc['B_proxy_rt_diff_B4_B3'] = df_proc['B4-2'] - df_proc['B3-2']
        b_proxy_cols.append('B_proxy_rt_diff_B4_B3')
        
    # B9(다중과제) 시각 에러 / 청각 정확도 (멀티태스킹 압박)
    if 'B9_visual_error_rate' in df_proc.columns and 'B9_audio_accuracy' in df_proc.columns:
        df_proc['B_proxy_multi_B9'] = df_proc['B9_visual_error_rate'] / (df_proc['B9_audio_accuracy'] + 1e-6)
        b_proxy_cols.append('B_proxy_multi_B9')
        
    # B10(고급 다중과제) 시각 에러 / 청각 정확도 (고급 멀티태스킹 압박)
    if 'B10_total_visual_error_rate' in df_proc.columns and 'B10_audio_accuracy' in df_proc.columns:
        df_proc['B_proxy_multi_B10'] = df_proc['B10_total_visual_error_rate'] / (df_proc['B10_audio_accuracy'] + 1e-6)
        b_proxy_cols.append('B_proxy_multi_B10')
        
    df_proc[b_proxy_cols] = df_proc[b_proxy_cols].fillna(0.0)
    print(f"[Global FE V14] Created {len(b_proxy_cols)} proxy features.")
    # --- ⬆️ [V14] B-피처 상호작용 (A9 프록시) ⬆️ ---
...
def train_single_fold(
    ...
    if which == "A":
        ...
        # (A피처 + [V13] 플래그 피처 + [V14] 프록시 피처가 함께 학습됨)
...
def save_meta():
    meta = dict(
        model=f"HGB({N_SPLITS_KFold}-Fold Ensemble) + 3-seed AvgProba + OrdinalEnc + Calib [Optuna V14-ProxyFE]", # 이름 수정
        feature_engineering=[
            ...
            "[V13-FE] Added Missingness Indicator flags (Key_Has_A, Key_Has_B_AB)",
            "[V13.4-FE] Dropped 'Test' from A/B.csv to prevent merge collision",
            "[V14-FE] Added B-feature interaction proxies (rt_diff, multi_B9/B10)", # <-- 수정됨
...
def main():
    ...
    save_meta() # [V14] 메타 정보 저장

    dt = time.time() - t0
    print(f"[V14-ProxyFE] submission saved -> {SUBMISSION_PATH} | elapsed: {dt:.2f}s")

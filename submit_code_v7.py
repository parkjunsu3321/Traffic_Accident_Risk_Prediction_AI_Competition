...
# [V7] Calibrator 생성 로직 수정
def _mk_calibrator(base_clf, use_prefit: bool):
    """
    use_prefit: True이고 sklearn 1.4+ 이면, cv="prefit" 사용
    base_clf: use_prefit=True이면 (학습된 모델), False이면 (학습 안된 모델)
    """
    try:
    ...
    if use_prefit and (major, minor) >= (1, 4):
        # [V7] 'prefit' 사용. estimator에 이미 학습된 모델 전달
        kw["cv"] = "prefit"
        return CalibratedClassifierCV(estimator=base_clf, **kw)
    else:
        # [V7] 'prefit' 미사용 또는 구버전. 
        # base_clf가 이미 학습된 모델이라도, CalibratedClassifierCV가 내부에서 clone하여 새로 학습함.
        if use_prefit:
...
# [V7] Calibrator 학습 로직 수정
def maybe_calibrate(base_clf_fitted, X_val, y_val):
    """
    이미 학습된 base_clf_fitted를 X_val, y_val을 사용해 보정합니다.
    """
    if not USE_CALIBRATION:
        return base_clf_fitted
    
    # [V7] 'prefit'=True로 하여 X_val, y_val로 보정만 수행 시도
    calib = _mk_calibrator(base_clf_fitted, use_prefit=True)
    
    try:
        # [V7] 보정은 반드시 검증 데이터(X_val)로 수행
        calib.fit(X_val, y_val) 
        return calib
    except Exception as e:
        # prefit이 실패하거나 (e.g., 구버전 sklearn) X_val이 없는 경우
...
# -------------------\
# [V7] 학습/로드 (Optuna 'max_iter' 튜닝 적용)
# -------------------\
def fit_or_load(
    ...
    if len(df_idx) and label_col in df_idx.columns:
        ...
        def objective(trial):
            ...
            # [V7] Optuna 탐색 시에는 early_stopping을 사용하지 않음
            # GKF로 분리된 X_val_t로만 평가해야 누수가 없음
        ...
        # [V7] Optuna가 찾은 모든 파라미터 (max_iter 포함)로 전역 설정 업데이트
        BASE_HGB_PARAMS.update(best_params)
        
        # [V7] 앙상블 학습 시에도 early_stopping은 끈 상태를 유지 (누수 방지)
        ...
        for sd in ENSEMBLE_SEEDS:
            # [V7] Optuna가 찾은 최적의 max_iter가 적용된 모델 생성
            base = build_model(sd).fit(X_tr_t, y_tr)
            
            # [V7] 보정(Calibration)은 X_val_t, y_val로 수행
...
def save_meta():
    meta = dict(
        model="HGB(3-seed soft ensemble) + OrdinalEnc + Calibration [Optuna V7.1 - TimeSeries FE + GroupKFold + max_iter tuning]", # [V7.1] 모델명
        feature_engineering=[
        ...
        validation_strategy=f"GroupKFold (n_splits={N_SPLITS_KFold}, used split 0) on PrimaryKey", # [V6]
        
        # [V7] Optuna가 튜닝한 최종 파라미터 (max_iter 포함) 저장
        ...
        calib_cv=f"{CALIB_CV} (fallback) or 'prefit' (if sk-ver >= 1.4)", # [V7]
        sklearn_version=sklver,
        random_state=RANDOM_STATE,
    )
    # [V7] BASE_HGB_PARAMS가 튜닝 후 변경되므로, NoneType 등을 json으로 저장하기 위해 str로 변환
...
def main():
    ...
    dt = time.time() - t0
    print(f"[V7.1] submission saved -> {SUBMISSION_PATH} | elapsed: {dt:.2f}s")
